...
<script>
    //메인 버튼을 클릿했을 때 메인 페이지로 이동하는 이벤트 처리 
	document.getElementById("mainbtn").addEventListener("click", function(e){
		location.href= "../";
	});
    
    //form이 있는 경우에는 form의 데이터가 전송될 때 유효성 검사를 수행해 주어야 합니다. 
    //유효성 검사는 클라이언트 특에서도 하고 서버측에서도 하는 것이 좋습니다. 
    //클라이언트 측에서 하면 보안은 취약하지만 서버에게 부담을 주지 않고 유효성 검사를 할 수 있습니다. 
    //클라이언트 측에서 유효성 검사를 수행했지만 서버에서 다시 하게 되는데 이유는 전송 도중에 데이터가
    //변조 되었을 수도 있기 때문입니다. 
    //로그인의 경우는 일반적인 유효성검사에 더해서 SQL의 예약어가 포함되어 있는지 확인하는 것이 좋습니다. 
    //or가 포함되어 있는지 확인 
