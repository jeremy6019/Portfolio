...
public class BoardDao {
	...
	private BoardDao() {
		try {
			...
			//ConnectionPool을 이용해서 Connection 만들기 
			Context init = new InitialContext();
			DataSource ds = (DataSource)init.lookup("java:comp/env/DBConn");
			con = ds.getConnection();
			
			//트랜잭션 관련 메소드를 직접 호출하기 위해서 autoCommit해제 
			//데이터베이스 프로그래밍을 하다가 작업한 곳에서는 데이터작업이 반영되는데 
			//원본 데이터베이스에는 반영이 안되어 있으면 commit여부를 확인해야 합니다. 
	...
	//DTO클래스를 매개변수로 받아서 영향받는 행의 개수를 리턴 
	...
	//id와 password를 받아서 mem테이블에 저장하는 메소드 
